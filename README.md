...
首次使用时如果本机还没有密钥缓存，程序会在“正在自动获取数据库密钥”阶段使用已登录的电脑版微信自动获取数据库密钥，再继续解密和导出；不需要手动填写 key。首次获取必须保持微信登录、微信进程未退出，并确保已经进入过微信主界面。成功后，数据库密钥和图片密钥只保存到当前 Windows 用户的 `%APPDATA%\wxMoments\runtime\output\account_keys.json`，不会写入 EXE、GitHub 仓库或导出 HTML。
...
发布或分享程序时只需要提供 `dist\wxMoments.exe` 和使用说明。不要把运行后的 `runtime\`、`output\`、`exports\` 目录或 `%APPDATA%\wxMoments\` 一起打包；这些目录可能包含本机账号路径、密钥缓存和朋友圈数据。当前仓库的发布 EXE 不包含 `account_keys.json`、本机账号名或朋友圈内容。
