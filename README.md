...
这是一个面向 **Windows 10/11 64 位** 的微信朋友圈本地缓存导出工具，程序名称为 **微信朋友圈导出工具1.01**。它从电脑版微信已经写入本机的缓存中读取朋友圈，按年份分别生成 HTML；每年页面中的照片都可以点击打开原图。
...
- HTML 中的照片支持点击打开原图。
- 可勾选并选择手动的 `xwechat_files` 数据目录；手动目录无效或找不到账号时自动回退快速检索。
...
4. 如果微信把数据放在自定义位置，勾选“使用手动 xwechat_files 目录”，点击“选择目录…”并选择该目录。手动目录不可用或里面找不到账号时，程序会自动使用原来的快速检索。
5. 如果需要先加载历史缓存，在“朋友圈辅助刷新”区域启用 F9 辅助刷新。
6. **先切换到微信朋友圈界面，并点击左上角切换到最早的一条。**
7. 按 `F9` 开始辅助刷新；程序会按照间隔发送 PgUp。
8. 再次按 `F9` 结束辅助刷新。微信窗口必须保持在前台；切换到其他窗口后辅助刷新会暂停。
9. 等微信缓存加载完成后，点击“开始逐年导出”。
10. 大量内容导出时可以点击“停止导出”中断抓取。
...
程序默认优先检查常见的 `xwechat_files`、`WeChat Files`、`Weixin Files` 和微信应用数据目录，再对当前用户目录下名称明显属于微信的目录做有限范围兜底检测。程序不会再把所有磁盘根目录作为起点递归扫描，因此“正在自动定位微信账号”通常会很快结束。勾选手动目录后，程序先检查用户选择的位置；目录不存在、没有找到账号或无法使用时会清空本次定位配置并回到上述自动快速检索。数据库密钥不显示在设置中，成功获取后只保存在本机运行目录中。
