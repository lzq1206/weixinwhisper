...
这是一个面向 **Windows 10/11 64 位** 的微信朋友圈本地缓存导出工具。它从电脑版微信已经写入本机的缓存中读取朋友圈，按年份分别生成 HTML；每年页面中的照片都可以点击打开原图。
...
程序会尝试识别常见的 `xwechat_files`、`WeChat Files` 和 `Weixin Files` 目录，并扫描可访问的账号目录。数据库密钥不显示在设置中，成功获取后只保存在本机运行目录中。
