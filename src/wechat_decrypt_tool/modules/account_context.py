...
from .constants import DB_KEY_HEX_LENGTH
...
def _has_decrypted_account_dbs(account_dir: Path) -> bool:
...
def _source_info_from_key_store(account: str) -> tuple[dict[str, Any], bool]:
    ...
    return source, len(str(keys.get("db_key") or "").strip()) == DB_KEY_HEX_LENGTH
...
def _key_state_from_key_store(account: str) -> dict[str, Any]:
    ...
    return {
        "db_key_present": len(str(keys.get("db_key") or "").strip()) == DB_KEY_HEX_LENGTH,
...
def _context_for_name(account: str) -> Optional[AccountContext]:
    ...
    try:
        if account_dir.exists() and account_dir.is_dir():
            has_dbs = _has_decrypted_account_dbs(account_dir)
...
def list_account_contexts() -> list[AccountContext]:
    names: set[str] = set()
    ...
    if isinstance(store, dict):
        for name, item in store.items():
            ...
            if not n or not isinstance(item, dict):
                continue
            has_key = len(str(item.get("db_key") or "").strip()) == DB_KEY_HEX_LENGTH
...
def list_account_names() -> list[str]:
    return [ctx.name for ctx in list_account_contexts()]


def resolve_account_context(account: Optional[str]) -> AccountContext:
    contexts = list_account_contexts()
    if not contexts:
        raise AccountError(
            status_code=404,
    ...
    if ctx is None:
        raise AccountError(status_code=404, detail="Account not found.")

    base = get_output_databases_dir().resolve()
    candidate = ctx.account_dir.resolve()
    if candidate != base and base not in candidate.parents:
        raise AccountError(status_code=400, detail="Invalid account path.")
