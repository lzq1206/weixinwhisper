...
from .constants import IMAGE_AES_KEY_LENGTH, IMAGE_XOR_KEY_MAX


V2_MAGIC = bytes((0x07, 0x08, 0x56, 0x32, 0x08, 0x07))
V2_CIPHERTEXT_START = 0x0F
AES_BLOCK_SIZE = IMAGE_AES_KEY_LENGTH
...
def derive_image_keys(code: int, wxid: str) -> DerivedImageKeys:
    ...
    return DerivedImageKeys(xor_key=code & IMAGE_XOR_KEY_MAX, aes_key=digest[:IMAGE_AES_KEY_LENGTH])
...
def _decrypt_aes_block(aes_key: str | bytes, ciphertext: bytes) -> bytes | None:
    try:
        ...
        return None
    if len(key_bytes) < IMAGE_AES_KEY_LENGTH or len(ciphertext) != IMAGE_AES_KEY_LENGTH:
        return None

    try:
        decryptor = Cipher(algorithms.AES(key_bytes[:IMAGE_AES_KEY_LENGTH]), modes.ECB()).decryptor()
...
def _infer_xor_key_with_support(tails: Iterable[bytes]) -> tuple[int | None, int]:
    ...
    first = pair[0] ^ IMAGE_XOR_KEY_MAX
    second = pair[1] ^ 0xD9
    ...
    for path in paths:
        ...
        try:
            with path.open("rb") as stream:
                header = stream.read(V2_CIPHERTEXT_START + IMAGE_AES_KEY_LENGTH)
                if len(header) < V2_CIPHERTEXT_START + IMAGE_AES_KEY_LENGTH or not header.startswith(V2_MAGIC):
                    continue
        ...
        templates.append(
            V2Template(
                path=path,
                ciphertext=header[V2_CIPHERTEXT_START : V2_CIPHERTEXT_START + IMAGE_AES_KEY_LENGTH],
...
def verify_key_pair(
    ...
    *,
    ...
    if isinstance(xor_key, bool) or not isinstance(xor_key, int) or not (0 <= xor_key <= IMAGE_XOR_KEY_MAX):
        return False
...
def resolve_local_image_key(
    *,
    ...
    if current_xor_key is not None:
        ordered_codes.sort(key=lambda code: (code & IMAGE_XOR_KEY_MAX != current_xor_key, code))
