...
from .media_cache import _resolve_account_dir, _resolve_account_wxid_dir

from .constants import INTERNAL_DB_KEY_BYTE_LENGTH, IMAGE_XOR_KEY_MAX
...
def _normalize_db_key(value: Any) -> str:
    ...
    if isinstance(value, (bytes, bytearray)):
        ...
        if len(raw_bytes) == INTERNAL_DB_KEY_BYTE_LENGTH:
            key = raw_bytes.hex()
        else:
            try:
...
def _normalize_internal_db_key(value: Any) -> bytes:
    ...
    if not cleaned:
        return b""
    if len(cleaned) != INTERNAL_DB_KEY_BYTE_LENGTH * 2:
        raise RuntimeError(
...
def _load_internal_db_key_candidates(wechat_install_path: Optional[str] = None) -> list[bytes]:
    ...
    for item in candidates:
        ...
        try:
            ...
            continue
        if len(key_bytes) == INTERNAL_DB_KEY_BYTE_LENGTH and key_bytes not in keys:
...
def _parse_image_xor_key(value: Any) -> Optional[int]:
    ...
    if isinstance(value, int):
        return value if 0 <= value <= IMAGE_XOR_KEY_MAX else None
    ...
    try:
        ...
        return None
    return parsed if 0 <= parsed <= IMAGE_XOR_KEY_MAX else None
...
async def get_image_key_integrated_workflow(
    ...
    raise RuntimeError("无法在本机取得已验证的图片密钥，请保持微信已登录后重试")
